...
class CollectiveInteraction(Interaction):
    ...
    def single_time_step_for_group(self, group):
        ...
        for recipient in group.get_susceptible():
            self.single_time_step_for_recipient(recipient, transmission_probability, group)

    def single_time_step_for_recipient(self, recipient, transmission_probability, group):
        ...
        if recipient_probability > 0.0:
            if random.random() <= transmission_probability * recipient_probability:
                #TODO fix this
                self.world.initialize_infection(recipient)
                #recipient.set_infection(
                #    self.selector.make_infection(recipient, self.world.timer.now)
                #)
    ...
    def added_transmission_probability(self, group):
        ...
        interaction_intensity = (
            group.get_intensity() / max(group.size() - 1, 1) * (self.world.timer.now - self.world.timer.before) 
        )
