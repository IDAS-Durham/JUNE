...
class InteractionStochastic(Interaction):
    def __init__(self, user_parameters, world):
        required_parameters = ["omega", "psi", "severe"]
    ...
    def calculate_localised_transmission_probability(self):
        intensity      = self.group.get_intensity(self.world.timer.now)
        ...
        return intensity * summed_prob * (self.world.timer.now - self.world.timer.previous)
    ...
    def single_time_step_for_recipient(self,recipient,transmission_probability):
        ...
        if recipient_probability > 0.0:
            if random.random() <= 1.-np.exp(-transmission_probability * recipient_probability):
                ...
                recipient.get_counter().update_infection_data(self.world.timer.now,self.group.get_spec())
                disc = random.random()
                i    = 0
                ...
                # this is now in the defaults config
