...
def _get_age_brackets(nomis_age_bin):
    try:
...
class BoxGenerator(Box):
    ...
    def create_box_from_region(self, region):
        ...
        # sex numbers
        number_of_men = (
            inputs.n_residents * np.array(inputs.sex_freq["males"]).flatten()
        )
        number_of_men = number_of_men.sum()
        number_of_women = (
            inputs.n_residents * np.array(inputs.sex_freq["females"]).flatten()
        )
        ...
        age_counts_total = np.array(age_counts.values) * np.array(inputs.n_residents.values)
        # and sum over all areas
        age_counts_total = age_counts_total.sum(axis=0).astype(np.int)
        # finally divide by total population
        ...
        for i, age_bin in enumerate(age_bins):
            ...
            age_1, age_2 = _get_age_brackets(age_bin)
            if age_1 == age_2:
                age_array = age_1 * np.ones(age_counts_total[i])
            else:
        ...
        for _ in range(number_of_men):
            ...
            person = Person(world=self.world, age=age, sex=sex, health_index=health_index)
            counter += 1
    ...
    def create_random_box(self, n_people):
        ...
        people_per_age_array = people_per_age * np.ones(len(selected_ages), dtype=np.int)
        # fill remaining people in random age groups
        remaining_people = n_people % len(selected_ages) 
        if remaining_people != 0:
            random_ages = np.random.choice(possible_ages, remaining_people, replace=False)
