...
def create_household_distributor():
    hd = HouseholdDistributor(distribution_kids_parents_age, distribution_couples_age)
    return hd
...
def test__get_matching_parent_is_correct():
    kid = Person(age=20)
    hdistributor = HouseholdDistributor(men_by_age=men_by_age, women_by_age=women_by_age)
    matching_kids = []
    for _ in range(50):
