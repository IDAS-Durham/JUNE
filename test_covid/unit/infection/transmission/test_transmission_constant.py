...
def make_transmission_constant_defaults(test_timer):
    trans = TransmissionConstant(test_timer)
    return trans
...
def test_update_probability(tdefault, test_timer):
    tdefault.update_probability()
    assert(tdefault.last_time_updated == test_timer.now)

@pytest.fixture(name="tuser")
def make_transmission_constant(test_timer, user_parameters):
    trans = TransmissionConstant(test_timer, user_parameters)
    return trans
