...
def test__smaller_than_one():
    ...
    for i in range(len(index_list.prob_lists[0])):
        index_m=index_list(i,0)
        index_w=index_list(i,1)
...
def test__growing_index():
    ...
    for i in range(len(index_list.prob_lists[0])):
        index_m=index_list(i,0)
        index_w=index_list(i,1)
        bool_m=all(i < j for i, j in zip(index_list.prob_lists[0][0], index_list.prob_lists[0][0][1:])) 
        bool_w=all(i < j for i, j in zip(index_list.prob_lists[0][0], index_list.prob_lists[0][0][1:]))
