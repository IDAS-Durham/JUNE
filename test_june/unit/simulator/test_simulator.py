...
def test__apply_activity_hierarchy(sim: Simulator):
    unordered_activities = random.sample(
        activity_hierarchy, len(activity_hierarchy)
    )
    ordered_activities = sim.activity_manager.apply_activity_hierarchy(unordered_activities)
...
def test__clear_world(sim: Simulator):
    sim.clear_world()
    for group_name in sim.activity_manager.activities_to_groups(sim.activity_manager.all_activities):
...
def test__move_people_from_hospital_to_icu(sim: Simulator):
    dummy_person = sim.world.people.members[0]
...
def test__move_people_from_icu_to_hospital(sim: Simulator):
    dummy_person = sim.world.people.members[0]
    dummy_person.health_information.infection.symptoms.tag = SymptomTag.hospitalised
