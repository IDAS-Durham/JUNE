...
class Groceries:
    ...
    def init_members(self,ratio):
        index = 0
        for super_area in self.geography.super_areas.members:
            self.fill_super_area_with_groceries(super_area,ratio,index)
            
    def fill_super_area_with_groceries(self,super_area,ratio,index):
        n_residents = 0
        ...
        groceries = []
        while n_actual>0:
            groceries.append(Grocery(index,super_area.coordinates))
            index    += 1
            n_actual -= 1
        self.members[super_area] = groceries
        
class GroceryFiller:
    def __init__(self,groceries,timer):
        self.groceries = groceries
        self.timer     = timer
        self.init_parameters()
        
    def init_parameters(self):
        # these parameters steer the sociology of the grocery customers
        self.daily_probability = 0.15
        self.male_ratio        = 0.33
        self.full_household    = 0.33
        self.male_probability  = self.male_ratio/(1.+self.male_ratio) 

    def send_people_to_groceries(self):
        for super_area, groceries in self.groceries.members.items():
            for area in super_area.areas:
    ...
    def send_people_in_household_to_grocery(self,household,grocery):
        candidates = []
        young_kids = []
        for candidate in household.people:
            if (candidate.active_group==None and
                candidate.age>=18 and
                (candidate.sex=="f" or
                 (candidate.sex=="m" and random.random()<self.male_probability))):
                candidates.append(candidate)
            elif candidate.age<12:
                young_kids.append(candidate)
        if len(candidates)>1 and random.random()>self.full_household:
            canholder = random.sample(household.people,1)[0]
            candidates = [canholder]
        if candidates==[]:
            canholder = random.sample(household.people,1)[0]
            if canholder.active_group!=None:
                return
            candidates.append(canholder)
        for person in candidates+young_kids:
            grocery.add(person, Grocery.GroupType.customers)
