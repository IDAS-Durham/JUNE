...
class CommuteHubs:
    ...
    def _get_msoa_lat_lon(self, msoa):
        ...
        msoa_lat = float(
            self.msoa_coordinates["Y"][self.msoa_coordinates["MSOA11CD"] == msoa]
        )
        msoa_lon = float(
            self.msoa_coordinates["X"][self.msoa_coordinates["MSOA11CD"] == msoa]
        )
        ...
        for commutecity in self.commutecities:
            ...
            for hub in hubs:
                commute_hub = CommuteHub(
                    commutehub_id=ids, lat_lon=hub, city=commutecity.city
                )
