...
class TravelUnitDistributor:
    ...
    def distribute_people_out(self):
        ...
        for idx, travelcity in enumerate(self.travelcities):
            if travelcity.city == "London":
                to_distribute_global = list(
                    self.to_distribute_df[
                        self.to_distribute_df["station"] == travelcity.city
                    ]["average_no_commute"]
                )[0] * (1 - self.configs["London damping factor"])
            else:
                to_distribute_global = list(
                    self.to_distribute_df[
                        self.to_distribute_df["station"] == travelcity.city
                    ]["average_no_commute"]
                )[0] * (1 - self.configs["non-London damping factor"])

            to_distribute_per_city = to_distribute_global * np.array(
                self.distribution_df[travelcity.city]
            )
            ...
            for dest_city_idx, to_distribute in enumerate(to_distribute_per_city):
                ...
                if not travel_msoas.size:
                    ...
                    msoas = travel_msoas[
                        np.random.choice(len(travel_msoas), int(to_distribute))
                    ]
                ...
                travel_unit = TravelUnit(city=travelcity.city,)

                for msoa_idx, msoa_name in enumerate(unique_msoa_names):

                    msoa = msoas[np.where(msoa_names == msoa_name)[0][0]]

                    people = np.array(msoa.people)[
                        np.random.choice(
                            len(msoa.people), counts[msoa_idx], replace=False
                        )
                    ]
                    ...
                    for person in people:
                        ...
                        if travel_unit.no_passengers < travel_unit.max_passengers:
                            ...
                            self.travelunits.add(travel_unit)
    ...
    def distribute_people_back(self):
        ...
        travel_cities = []
        for idx, travelcity in enumerate(self.travelcities):
            units.append(TravelUnit(city=travelcity.city,))
