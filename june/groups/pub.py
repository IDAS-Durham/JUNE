...
class Pub(Group):
    ...
    def __init__(self, pub_id=0,position=None):
        super().__init__()
        self.pub_id   = pub_id
...
class Pubs:
    ...
    def init_members(self,pub_df):
        if self.box_mode:
            ...
            for super_area in self.geography.super_areas.members:
                self.fill_areas_pub_map(super_area, pub_tree, pub_list)
    ...
    def fill_areas_pub_map(self,super_area,pub_tree,pub_list):
        angles, indices = pub_tree.query(
            np.deg2rad(super_area.coordinates.reshape(1, -1)), 20, sort_results=True
        )
        pubs = []
        ...
        self.members[super_area] = pubs

            
class PubFiller:
    def __init__(self, pubs,timer):
        self.pubs  = pubs
        self.timer = timer 
        self.init_parameters()
    ...
    def send_people_to_pub(self):
        for super_area, pubs in self.pubs.members.items():
            npeople = self.fix_number_of_customers(super_area)
            while npeople>0:
                pub      = np.random.choice(pubs)
                customer = self.select_customer(super_area)
                pub.add(customer, Pub.GroupType.guests)
                self.add_household_members(pub, customer)
                npeople -= 1
            
    def fix_number_of_customers(self, super_area):
        nmean = len(super_area.adult_active_females)+len(super_area.adult_active_males)
        if self.timer.weekend:
            nmean *= self.pub_weekend_ratio
        else:
            nmean *= self.pub_weekday_ratio
        npeople = round(np.random.poisson(nmean))
        return npeople
    
    def select_customer(self,super_area):
        customer = None
        while customer==None:
            if random.random()<self.pub_female_probability:
                customer = random.sample(super_area.adult_active_females,1)[0]
            else:
                customer = random.sample(super_area.adult_active_males,1)[0]
    ...
    def add_household_members(self,pub,customer):
        ...
        elif (not self.timer.weekend and
