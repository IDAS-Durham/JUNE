...
default_population_file = paths.data_path / 'input/health_index/population_by_age_2020_ew.csv'
...
class Data2Rates():
    ...
    def __init__(self, seroprevalence_df: pd.DataFrame, population_by_age_sex_df: pd.DataFrame):
...
if __name__=='__main__':
    ...
    print(rates.population_by_age_sex_df)
    #***************** Ward et al IFR
    ifr_age =  [0.03,0.52,3.13,11.64]
    ages = [pd.Interval(15,44,closed='left'),
    ...
    #***************** Imperial IFR
    ifr_age =  [0.,0.01,0.01,0.02,0.03,0.04,0.06,0.1,0.16,0.24,0.38,
               0.6,0.94,1.47,2.31,3.61,5.66,8.86,
               17.37]
    ...
    #******************* IFR comparison
    plt.bar(x=[(index.left+index.right)/2 for index in ifr_ward.index],
           height=ifr_ward[0].values,
           width=[index.right-index.left for index in ifr_ward.index],
           alpha=0.4,
           label='Ward')
    plt.bar(x=[(index.left+index.right)/2 for index in ifr_imperial.index],
