...
def distribute_passengers(city_travel, peak_commute=None, subtract_commute=False):
    ...
    city_travel["average"] = np.array(city_travel["average"]) / 100.0
    # Initialise
    ...
    if subtract_commute == False:
        ...
        city_travel["average_no_commute"] = (
            np.array(city_travel["average_no_commute"]) / 100.0
        )
    ...
    while finished == False:
        ...
        city_to_depart[city_to_depart <= 0.0] = 1.0
        ...
        for station_idx, num_to_travel in enumerate(departing):
            ...
            if subtract_commute == False:
                travel_average = np.delete(
                    np.array(city_travel["average"]), station_idx
                )
            else:
                travel_average = np.delete(
                    np.array(city_travel["average_no_commute"]), station_idx
                )
            travel_arrived = np.delete(np.array(city_travel["arrived"]), station_idx)
            travel_to_depart = np.delete(
                np.array(city_travel["to_depart"]), station_idx
            )
            ...
            # Hacky fix
            travel_to_arrive[travel_to_arrive <= 0.0] = 1.0
...
if __name__ == "__main__":
    ...
    travel_matrix, _ = distribute_passengers(
        city_travel=city_travel, peak_commute=peak_commuters, subtract_commute=True
    )
