...
class Plotter:
    ...
    @classmethod
    def from_file(
        cls, world_filename: str = default_world_filename,
    ):
    ...
    def plot_demography(
        self, save_dir: Path = default_output_plots_path / "demography", colors=colors,
    ):
        ...
        london_superareas_path = (
            ...
            / "example_scripts/london_areas.txt"
        )
        ...
        mean_plot = demography_plots.plot_socioeconomic_index(
            super_areas, column="centile_mean"
        )
        mean_plot.plot()
        plt.savefig(
        ...
        ethnicity_plot.plot()
        plt.savefig(
            save_dir / "ethnicity_distribution.png", dpi=150, bbox_inches="tight"
        )
        ...
        plt.plot()
        plt.savefig(
            save_dir / "regional_age_comparison.png", dpi=150, bbox_inches="tight"
        )

    def plot_commute(
        self, save_dir: Path = default_output_plots_path / "commute", colors=colors,
    ):
        ...
        (
            internal_commute_areas,
            external_commute_areas,
        ) = commute_plots.process_internal_external_areas(
            city_to_plot="Newcastle upon Tyne"
        )

        if internal_commute_areas is not None:
            print("Plotting Newcastle internal areas")
            commute_areas_plot = commute_plots.plot_commute_areas(
                internal_commute_areas
            )
        ...
        (
            internal_commute_areas,
            external_commute_areas,
        ) = commute_plots.process_internal_external_areas(city_to_plot="Birmingham")
        ...
        if internal_commute_areas is not None:
            ...
            commute_areas_plot.plot()
            plt.savefig(
                save_dir / "Birmingham_internal_commute.png",
                dpi=150,
                bbox_inches="tight",
            )
        ...
        if external_commute_areas is not None:
            ...
            commute_areas_plot = commute_plots.plot_commute_areas(
                external_commute_areas, figsize=(6, 8)
            )
            commute_areas_plot.plot()
            plt.savefig(
                save_dir / "Birmingham_external_commute.png",
                dpi=150,
                bbox_inches="tight",
            )
        ...
        (
            internal_commute_areas,
            external_commute_areas,
        ) = commute_plots.process_internal_external_areas(city_to_plot="London")
        ...
        if external_commute_areas is not None:
            ...
            commute_areas_plot.plot()
            plt.savefig(
                save_dir / "London_external_commute.png",
                dpi=150,
                bbox_inches="tight",
                figsize=(6, 8),
            )
    ...
    def plot_households(
        self, save_dir: Path = default_output_plots_path / "households", colors=colors,
    ):
        ...
        plt.savefig(
            save_dir / "household_sizes_ratios.png", dpi=150, bbox_inches="tight"
        )

        print("Plotting household probability matrix")
        household_probability_matrix = (
            household_plots.plot_household_probability_matrix()
        )
    ...
    def plot_companies(
        self, save_dir: Path = default_output_plots_path / "companies", colors=colors,
    ):
    ...
    def plot_leisure(
        self, save_dir: Path = default_output_plots_path / "leisure", colors=colors,
    ):
    ...
    def plot_policies(
        self, save_dir: Path = default_output_plots_path / "policy", colors=colors,
    ):
    ...
    def plot_care_homes(
        self, save_dir: Path = default_output_plots_path / "care_homes", colors=colors,
    ):
    ...
    def plot_schools(
        self, save_dir: Path = default_output_plots_path / "schools", colors=colors,
    ):
    ...
    def plot_health_index(
        self, save_dir: Path = default_output_plots_path / "health_index", colors=colors
    ):
        "Plot socioeconomic_index vs. life_expectancy"
        save_dir.mkdir(exist_ok=True, parents=True)
    ...
    def plot_universities(
        ...
        uni_map.plot()
        plt.savefig(
            save_dir / "student_household_locations.png",
            dpi=150,
            pad_inches=0,
            bbox_inches="tight",
        )
